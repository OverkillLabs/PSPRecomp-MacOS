...
- **PC control prompts.** The game's on-screen overlays and prompts now show your keyboard and mouse controls (for example the key or mouse button to press) instead of the PSP buttons. With a controller they show Xbox button names, and they switch automatically as you change device. Turn it off with "Show control names in prompts" to get the original PSP button names. The launcher's Controls page lists every keyboard, mouse and gamepad control.
